...
The default scheduled RDAP budget remains 50. The hunter computes the revised investor-style quality score with neutral history for every locally acceptable candidate, using free `wordfreq` frequency data and `wordninja` compound segmentation to distinguish common English words, uncommon but valid commercial terms, pronounceable coined brands, and random fragments. It rewards short one-word names, selective natural adjective+noun, noun+noun, verb+noun, and commercial category+product combinations, while penalizing awkward order, generic suffix stuffing, long three-or-more-word names, unrecognized tokens, spam patterns, and trademark-risk signals. The TOP 50 are selected by this quality score, not by source order or RDAP outcome. RDAP enriches up to 50 TOP 50 entries; a high-quality candidate with UNKNOWN status remains in the report and is never described as available.
